    ...
    class HashDB : public BasicDB
    {
        ...
        public:
            ...
            class Cursor : public BasicDB::Cursor
            {
                ...
                public:
                    ...
                    bool accept ( Visitor *visitor, bool writable = true, bool step = false )
                    {
                        ...
                        {
                            if ( !db_->defrag_impl ( db_->dfunit_ * DFRGCEF ) )
                            {
                                return false;
                            }
            ...
            bool accept ( const char *kbuf, size_t ksiz, Visitor *visitor, bool writable = true )
            {
                ...
                if ( !accept_impl ( kbuf, ksiz, visitor, bidx, pivot, false ) )
                {
                    err = true;
                }
                rlock_.unlock ( lidx );
            ...
            bool iterate ( Visitor *visitor, bool writable = true, ProgressChecker *checker = NULL )
            {
                ...
                bool err = false;
                if ( !iterate_impl ( visitor, checker ) )
                {
                    err = true;
                }
            ...
            bool scan_parallel ( Visitor *visitor, size_t thnum, ProgressChecker *checker = NULL )
            {
                ...
                bool err = false;
                if ( !scan_parallel_impl ( visitor, thnum, checker ) )
                {
                    err = true;
                }
                rlock_.unlock_all();
                             ...
                             Error::Code code, const char *message )
            {
                ...
                if ( code == Error::BROKEN || code == Error::SYSTEM )
                {
                    flags_ |= FFATAL;
                }
                if ( logger_ )
            ...
            bool open ( const std::string &path, uint32_t mode = OWRITER | OCREATE )
            {
                ...
                if ( !file_.open ( path, fmode, msiz_ ) )
                {
                    ...
                    if ( std::strstr ( emsg, "(permission denied)" ) || std::strstr ( emsg, "(directory)" ) )
                    {
                        ...
                        if ( std::strstr ( emsg, "(file not found)" ) || std::strstr ( emsg, "(invalid path)" ) )
                        {
                            code = Error::NOREPOS;
                        }
                               ...
                               ProgressChecker *checker = NULL )
            {
                ...
                rlock_.lock_reader_all();
                bool err = false;
                if ( !synchronize_impl ( hard, proc, checker ) )
                {
                    err = true;
                }
        ...
        private:
                                    ...
                                    if ( vbuf == Visitor::REMOVE )
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                if ( !over )
                                                {
                                                    psiz_.secure_least ( rec.off + rec.rsiz );
                                                }
                                                delete[] zbuf;
                                                delete[] rec.bbuf;
                                                if ( entoff > 0 )
            ...
            bool synchronize_impl ( bool hard, FileProcessor *proc, ProgressChecker *checker )
            {
                ...
                if ( writer_ && !autotran_ && !set_flag ( FOPEN, true ) )
                {
                    err = true;
                }
                return !err;
            ...
            void calc_meta()
            {
                ...
                if ( fbpnum_ > 0 )
                {
                    boff_ += width_ * 2 + sizeof ( uint8_t ) * 2;
                }
            ...
            bool copy_records ( HashDB *dest )
            {
                ...
                while ( off > 0 && off < end )
                {
                    ...
                    if ( !read_record ( &rec, rbuf ) )
                    {
                        ...
                        if ( checkend > end - ( int64_t ) rhsiz_ )
                        {
                            checkend = end - rhsiz_;
                        }
                        bool hit = false;
            ...
            int32_t compare_keys ( const char *abuf, size_t asiz, const char *bbuf, size_t bsiz )
            {
                ...
                if ( asiz != bsiz )
                {
                    return ( int32_t ) asiz - ( int32_t ) bsiz;
                }
            ...
            bool adjust_record ( Record *rec )
            {
                ...
                if ( rec->psiz > ( size_t ) INT16MAX || rec->psiz > rec->rsiz / 2 )
                {
                    ...
                    char nbuf[RECBUFSIZ];
                    if ( !write_free_block ( noff, nsiz, nbuf ) )
                    {
                        return false;
                    }
            ...
            bool shift_record ( Record *orec, int64_t dest )
            {
                ...
                {
                    orec->off = dest;
                    if ( !write_record ( orec, true ) )
                    {
                        return false;
                    }
                    if ( !set_bucket ( bidx, dest ) )
                    {
                        return false;
                    }
                    return true;
                    ...
                    if ( rec->left < 1 && rec->right > 0 )
                    {
                        ...
                        else
                        {
                            ...
                            if ( prec.right > 0 )
                            {
                                ...
                                child = off;
                                if ( !set_chain ( pentoff, prec.left ) )
                                {
                                    return false;
                                }
                                if ( !set_chain ( off + sizeof ( uint16_t ), rec->left ) )
                                {
                                    return false;
                                }
                                if ( !set_chain ( off + sizeof ( uint16_t ) + width_, rec->right ) )
                                {
                                    return false;
                                }
                            }
                            ...
                            {
                                child = prec.off;
                                if ( !set_chain ( prec.off + sizeof ( uint16_t ) + width_, rec->right ) )
                                {
                                    return false;
                                }
                            }
            ...
            {
                _assert_ ( true );
                if ( ( count_ != trcount_ || lsiz_ != trsize_ ) && !dump_meta() )
                {
                    return false;
                }
            ...
            bool commit_transaction()
            {
                _assert_ ( true );
                bool err = false;
                if ( ( count_ != trcount_ || lsiz_ != trsize_ ) && !dump_auto_meta() )
                {
                    err = true;
                }
