...
namespace kyotocabinet                   // common namespace
{
    ...
    inline uint64_t hashmurmur ( const void *buf, size_t size )
    {
        ...
        {
            case 7:
                hash ^= ( uint64_t ) rp[6] << 48;
            case 6:
                hash ^= ( uint64_t ) rp[5] << 40;
            case 5:
                hash ^= ( uint64_t ) rp[4] << 32;
            case 4:
                hash ^= ( uint64_t ) rp[3] << 24;
            case 3:
                hash ^= ( uint64_t ) rp[2] << 16;
            case 2:
                hash ^= ( uint64_t ) rp[1] << 8;
            case 1:
                hash ^= ( uint64_t ) rp[0];
                hash *= mul;
        };
...
#define MAX_LINE 2048
    ...
    inline bool strbwm ( const char *str, const char *key )
    {
        ...
        {
            if ( i > slen || str[slen - i] != key[klen - i] )
            {
                return false;
            }
        }
