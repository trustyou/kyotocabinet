        ...
        if ( !argbrk && argv[i][0] == '-' )
        {
            if ( !strcmp ( argv[i], "--" ) )
            {
                ...
                if ( !strcmp ( argv[i], "-rnd" ) )
                {
                    ...
                    if ( !strcmp ( argv[i], "-etc" ) )
                    {
                        etc = TRUE;
                    }
                    else
                        if ( !strcmp ( argv[i], "-tran" ) )
                        {
                            tran = TRUE;
                        }
                        else
                            if ( !strcmp ( argv[i], "-oat" ) )
                            {
                                oflags |= KCOAUTOTRAN;
                            }
                            else
                                if ( !strcmp ( argv[i], "-oas" ) )
                                {
                                    oflags |= KCOAUTOSYNC;
                                }
                                else
                                    if ( !strcmp ( argv[i], "-onl" ) )
                                    {
                                        oflags |= KCONOLOCK;
                                    }
                                    else
                                        if ( !strcmp ( argv[i], "-otl" ) )
                                        {
                                            oflags |= KCOTRYLOCK;
                                        }
                                        else
                                            if ( !strcmp ( argv[i], "-onr" ) )
                                            {
                                                oflags |= KCONOREPAIR;
                                            }
                                            else
                           ...
                           int32_t oflags )
{
    ...
    for ( i = 1; !err && i <= rnum; i++ )
    {
        ...
        if ( vbuf )
        {
            ...
            if ( !rnd || kcidxecode ( idx ) != KCENOREC )
            {
                idxerrprint ( idx, __LINE__, "kcidxget" );
                err = TRUE;
            }
                ...
                case 4:
                    {
                        if ( cnt > 0 )
                        {
                            kclistremove ( list, rnd ? myrand ( cnt ) : cnt / 2 );
                        }
                        break;
                    }
                case 5:
                    {
                        if ( cnt > 0 )
                        {
                            kclistget ( list, rnd ? myrand ( cnt ) : cnt / 2, &size );
                        }
                        break;
                    }
                case 6:
                    {
                        if ( rnd ? myrand ( 100 ) == 0 : i % 127 == 0 )
                        {
                            kclistclear ( list );
                        }
                        break;
