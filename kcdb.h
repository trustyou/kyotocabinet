    ...
    class BasicDB : public DB
    {
        ...
        public:
            ...
            bool copy ( const std::string &dest, ProgressChecker *checker = NULL )
            {
                ...
                class FileProcessorImpl : public FileProcessor
                {
                    ...
                    private:
                        ...
                        {
                            File::Status sbuf;
                            if ( !File::status ( path, &sbuf ) )
                            {
                                return false;
                            }
                            if ( sbuf.isdir )
                            {
                                if ( !File::make_directory ( dest_ ) )
                                {
                                    return false;
                                }
                                bool err = false;
                                ...
                                if ( dir.open ( path ) )
                                {
                                    ...
                                    while ( !err && dir.read ( &name ) )
                                    {
                                        ...
                                        if ( dbuf )
                                        {
                                            if ( !File::write_file ( dpath, dbuf, dsiz ) )
                                            {
                                                err = true;
                                            }
                                            delete[] dbuf;
            ...
            int64_t increment ( const char *kbuf, size_t ksiz, int64_t num, int64_t orig = 0 )
            {
                ...
                if ( !accept ( kbuf, ksiz, &visitor, num != 0 || orig != INT64MIN ) )
                {
                    return INT64MIN;
                }
                num = visitor.num();
                ...
                class VisitorImpl : public Visitor
                {
                    ...
                    private:
                        const char *visit_full ( const char *kbuf, size_t ksiz,
                                                 const char *vbuf, size_t vsiz, size_t *sp )
                        {
                            if ( !ovbuf_ || vsiz != ovsiz_ || std::memcmp ( vbuf, ovbuf_, vsiz ) )
                            {
                                return NOP;
                            }
                            ok_ = true;
            ...
            int64_t set_bulk ( const std::map<std::string, std::string> &recs, bool atomic = true )
            {
                ...
                if ( atomic )
                {
                    ...
                    if ( !accept_bulk ( keys, &visitor, true ) )
                    {
                        return -1;
                    }
                    return keys.size();
                               ...
                               std::map<std::string, std::string> *recs, bool atomic = true )
            {
                ...
                if ( atomic )
                {
                    ...
                    VisitorImpl visitor ( recs );
                    if ( !accept_bulk ( keys, &visitor, false ) )
                    {
                        return -1;
                    }
                    return recs->size();
            ...
            static const char *typestring ( uint32_t type )
            {
                ...
                switch ( type )
                {
                    case TYPEVOID:
                        return "void";
                    case TYPEPHASH:
                        return "prototype hash database";
                    case TYPEPTREE:
                        return "prototype tree database";
                    case TYPESTASH:
                        return "stash database";
                    case TYPECACHE:
                        return "cache hash database";
                    case TYPEGRASS:
                        return "cache tree database";
                    case TYPEHASH:
                        return "file hash database";
                    case TYPETREE:
                        return "file tree database";
                    case TYPEDIR:
                        return "directory hash database";
                    case TYPEFOREST:
                        return "directory tree database";
                    case TYPEMISC:
                        return "miscellaneous database";
                }
                return "unknown";
            }
    };
