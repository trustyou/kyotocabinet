    ...
    class MapReduce
    {
        ...
        public:
            ...
            bool execute ( BasicDB *db, const std::string &tmppath = "", uint32_t opts = 0 )
            {
                ...
                if ( count < 0 )
                {
                    if ( db->error() != BasicDB::Error::NOIMPL )
                    {
                        return false;
                    }
                    count = 0;
                ...
                if ( info == typeid ( GrassDB ) )
                {
                    ...
                    if ( info == typeid ( TreeDB ) )
                    {
                        TreeDB *tdb = ( TreeDB * ) idb;
                        rcomp_ = tdb->rcomp();
                    }
                    else
                        if ( info == typeid ( ForestDB ) )
                        {
                ...
                if ( opts & XPARARED )
                {
                    redtasks_ = new ReduceTaskQueue;
                }
                if ( opts & XPARAFLS )
                {
                    flsths_ = new std::deque<FlushThread *>;
                }
                if ( opts & XNOLOCK )
                {
                    ...
                    if ( !err )
                    {
                        ...
                        if ( !cur->jump() && cur->error() != BasicDB::Error::NOREC )
                        {
                            err = true;
                        }
                        while ( !err )
                        {
                            ...
                            {
                                if ( cur->error() != BasicDB::Error::NOREC )
                                {
                                    err = true;
                                }
                                break;
                    ...
                    else
                    {
                        MapChecker mapchecker;
                        MapVisitor mapvisitor ( this, &mapchecker, count );
                        if ( !err && !db->iterate ( &mapvisitor, false, &mapchecker ) )
                        {
                            err = true;
                        }
                        if ( mapvisitor.error() )
                ...
                if ( !logf ( "clean", "closing the temporary databases" ) )
                {
                    err = true;
                }
                stime = time();
        ...
        private:
                ...
                public:
                    ...
                    void run()
                    {
                        if ( !mr_->logf ( "map", "started to flushing the cache: count=%lld size=%lld",
                                          ( long long ) cache_->count(), ( long long ) csiz_ ) )
                        {
                            err_ = true;
                        }
                        double stime = time();
            ...
            class MapVisitor : public BasicDB::Visitor
            {
                public:
                    ...
                    void visit_before()
                    {
                        ...
                        if ( mr_->csiz_ > 0 && !mr_->flush_cache() )
                        {
                            err_ = true;
                        }
            ...
            bool execute_reduce()
            {
                ...
                if ( !logf ( "reduce", "started the reduce process: scale=%lld", ( long long ) scale ) )
                {
                    err = true;
                }
                double stime = time();
                ...
                double etime = time();
                if ( !logf ( "reduce", "the reduce process finished: time=%.6f", etime - stime ) )
                {
                    err = true;
                }
                return !err;
    ...
    class IndexDB
    {
        ...
        public:
                ...
                while ( it != itend )
                {
                    ...
                    if ( strsplit ( *it, '=', &fields ) > 1 )
                    {
                        ...
                        if ( !std::strcmp ( key, "idxclim" ) || !std::strcmp ( key, "idxcachelimit" ) )
                        {
                            ...
                            if ( !std::strcmp ( key, "idxcbnum" ) || !std::strcmp ( key, "idxcachebuckets" ) )
                            {
                                cbnum = atoix ( value );
                            }
                            else
                                if ( !std::strcmp ( key, "idxdbnum" ) )
                                {
                                    dbnum = atoix ( value );
                                }
                                else
                                    if ( !std::strcmp ( key, "idxtmppath" ) )
                                    {
                                        tmppath = value;
                                    }
                    }
            ...
            bool set ( const char *kbuf, size_t ksiz, const char *vbuf, size_t vsiz )
            {
                ...
                bool err = false;
                if ( !clean_dbs ( kbuf, ksiz ) )
                {
                    err = true;
                }
                cache_->set ( kbuf, ksiz, vbuf, vsiz );
                csiz_ += ksiz + vsiz;
                if ( csiz_ > clim_ && !flush_cache() )
                {
                    err = false;
                }
                return !err;
            ...
            bool synchronize ( bool hard = false, BasicDB::FileProcessor *proc = NULL )
            {
                ...
                if ( tmpdbs_ && !merge_tmpdbs() )
                {
                    err = true;
                }
                if ( !db_.synchronize ( hard, proc ) )
                {
                    err = true;
                }
                return !err;
        ...
        private:
            ...
            bool merge_tmpdbs()
            {
                ...
                double stime = time();
                if ( !db_.merge ( tmpdbs_, dbnum_, PolyDB::MAPPEND ) )
                {
                    err = true;
                }
                dbclock_ = 0;
            ...
            bool clean_dbs ( const char *kbuf, size_t ksiz )
            {
                ...
                bool err = false;
                if ( db_.error() != BasicDB::Error::NOREC )
                {
                    err = true;
                }
                if ( tmpdbs_ )
