    ...
    class PlantDB : public BasicDB
    {
        ...
        public:
            ...
            class Cursor : public BasicDB::Cursor
            {
                ...
                public:
                    ...
                    bool accept ( Visitor *visitor, bool writable = true, bool step = false )
                    {
                        ...
                        if ( lid_ > 0 && !accept_spec ( visitor, writable, step, &hit ) )
                        {
                            err = true;
                        }
                        ...
                        if ( !err && !hit )
                        {
                            ...
                            if ( kbuf_ )
                            {
                                ...
                                while ( !err && retry )
                                {
                                    if ( !accept_atom ( visitor, step, &retry ) )
                                    {
                                        err = true;
                                    }
                                }
                    ...
                    bool jump_back()
                    {
                        ...
                        bool err = false;
                        if ( !set_position_back ( db_->last_ ) )
                        {
                            err = true;
                        }
                        return !err;
                    ...
                    bool jump_back ( const char *kbuf, size_t ksiz )
                    {
                        ...
                        if ( adjust_position() )
                        {
                            ...
                            {
                                bool hit = false;
                                if ( lid_ > 0 && !back_position_spec ( &hit ) )
                                {
                                    err = true;
                                }
                                if ( !err && !hit )
                            ...
                            if ( hit )
                            {
                                ...
                                if ( link || flush || async )
                                {
                                    ...
                                    if ( link )
                                    {
                                        ...
                                        if ( node )
                                        {
                                            if ( !db_->reorganize_tree ( node, hist, hnum ) )
                                            {
                                                err = true;
                                            }
                                        ...
                                        if ( flush )
                                        {
                                            ...
                                            if ( !db_->flush_leaf_cache_part ( lslot ) )
                                            {
                                                err = true;
                                            }
                                            InnerSlot *islot = db_->islots_ + idx;
                                            if ( islot->warm->count() > lslot->warm->count() + lslot->hot->count() + 1 &&
                                                    !db_->flush_inner_cache_part ( islot ) )
                                            {
                                                err = true;
                                            }
                                        }
                                    if ( async && !db_->fix_auto_synchronization() )
                                    {
                                        err = true;
                                    }
                                }
                                ...
                                {
                                    if ( !db_->fix_auto_transaction_leaf ( node ) )
                                    {
                                        err = true;
                                    }
                                }
                    ...
                    bool accept_atom ( Visitor *visitor, bool step, bool *retryp )
                    {
                        ...
                        *retryp = false;
                        ...
                        if ( rit != ritend )
                        {
                            ...
                            bool atran = db_->autotran_ && !db_->tran_ && node->dirty;
                            bool async = db_->autosync_ && !db_->autotran_ && !db_->tran_ && node->dirty;
                            if ( atran && !reorg && !db_->fix_auto_transaction_leaf ( node ) )
                            {
                                err = true;
                            }
                            if ( reorg )
                            {
                                ...
                                if ( atran && !db_->fix_auto_transaction_tree() )
                                {
                                    err = true;
                                }
                            }
                            else
                                if ( db_->cusage_ > db_->pccap_ )
                                {
            ...
            bool accept ( const char *kbuf, size_t ksiz, Visitor *visitor, bool writable = true )
            {
                ...
                bool err = false;
                int64_t id = node->id;
                if ( atran && !reorg && !fix_auto_transaction_leaf ( node ) )
                {
                    err = true;
                }
                if ( cusage_ > pccap_ )
                {
                    ...
                    if ( !clean_leaf_cache_part ( lslot ) )
                    {
                        err = true;
                    }
                    flush = true;
                ...
                if ( reorg )
                {
                    ...
                    if ( node )
                    {
                        if ( !reorganize_tree ( node, hist, hnum ) )
                        {
                            err = true;
                        }
                        if ( atran && !tran_ && !fix_auto_transaction_tree() )
                        {
                            err = true;
                        }
                    }
                    ...
                    if ( flush )
                    {
                        ...
                        int32_t idx = id % SLOTNUM;
                        LeafSlot *lslot = lslots_ + idx;
                        if ( !flush_leaf_cache_part ( lslot ) )
                        {
                            err = true;
                        }
                        InnerSlot *islot = islots_ + idx;
                        if ( islot->warm->count() > lslot->warm->count() + lslot->hot->count() + 1 &&
                                !flush_inner_cache_part ( islot ) )
                        {
                            err = true;
                        }
                        mlock_.unlock();
                               ...
                               bool writable = true )
            {
                ...
                while ( !err && kit != kitend )
                {
                    ...
                    if ( reorg )
                    {
                        ...
                        if ( atran && !fix_auto_transaction_tree() )
                        {
                            err = true;
                        }
                    }
                    ...
                    if ( async && !fix_auto_synchronization() )
                    {
                        err = true;
                    }
                    ++kit;
            ...
            bool iterate ( Visitor *visitor, bool writable = true, ProgressChecker *checker = NULL )
            {
                ...
                while ( !err && id > 0 )
                {
                    ...
                    {
                        Record *rec = *kit;
                        if ( accept_impl ( node, rec, visitor ) )
                        {
                            reorg = true;
                        }
                        curcnt++;
                    ...
                    if ( cusage_ > pccap_ )
                    {
                        ...
                        if ( islot->warm->count() > 2 && !flush_inner_cache_part ( islot ) )
                        {
                            err = true;
                        }
                    }
            ...
            bool scan_parallel ( Visitor *visitor, size_t thnum, ProgressChecker *checker = NULL )
            {
                ...
                class VisitorImpl : public Visitor
                {
                    ...
                    private:
                        ...
                        {
                            if ( ksiz < 2 || ksiz >= NUMBUFSIZ || kbuf[0] != LNPREFIX )
                            {
                                return NOP;
                            }
                            uint64_t prev;
                ...
                if ( !db_.scan_parallel ( &ivisitor, thnum, &ichecker ) )
                {
                    err = true;
                }
            ...
            bool open ( const std::string &path, uint32_t mode = OWRITER | OCREATE )
            {
                ...
                if ( !db_.tune_type ( DBTYPE ) )
                {
                    return false;
                }
                if ( !db_.tune_alignment ( apow_ ) )
                {
                    return false;
                }
                if ( !db_.tune_fbp ( fpow_ ) )
                {
                    return false;
                }
                if ( !db_.tune_options ( opts_ ) )
                {
                    return false;
                }
                if ( !db_.tune_buckets ( bnum_ ) )
                {
                    return false;
                }
                ...
                if ( db_.reorganized() )
                {
                    ...
                    if ( db_.recovered() )
                    {
                        ...
                        if ( count_ == INT64MAX && !reorganize_file ( mode ) )
                        {
                            return false;
                        }
                    }
                               ...
                               ProgressChecker *checker = NULL )
            {
                ...
                class Wrapper : public FileProcessor
                {
                    ...
                    private:
                        bool process ( const std::string &path, int64_t count, int64_t size )
                        {
                            if ( proc_ )
                            {
                                return proc_->process ( path, count_, size );
                            }
                            return true;
                        }
                        FileProcessor *proc_;
                ...
                if ( !db_.synchronize ( hard, &wrapper, checker ) )
                {
                    err = true;
                }
            ...
            bool status ( std::map<std::string, std::string> *strmap )
            {
                ...
                if ( reccomp_.comp == LEXICALCOMP )
                {
                    ...
                    if ( reccomp_.comp == DECIMALCOMP )
                    {
                        compname = "decimal";
                    }
                    else
                        if ( reccomp_.comp == LEXICALDESCCOMP )
                        {
                            compname = "lexicaldesc";
                        }
                        else
                            if ( reccomp_.comp == DECIMALDESCCOMP )
                            {
        ...
        protected:
            ...
            void trigger_meta ( MetaTrigger::Kind kind, const char *message )
            {
                ...
                if ( mtrigger_ )
                {
                    mtrigger_->trigger ( kind, message );
                }
            }
        private:
            /**
            ...
            bool save_leaf_node ( LeafNode *node )
            {
                ...
                if ( node->dead )
                {
                    if ( !db_.remove ( hbuf, hsiz ) && db_.error().code() != Error::NOREC )
                    {
                        err = true;
                    }
                }
                else
                {
                    ...
                    if ( !db_.set ( hbuf, hsiz, rbuf, wp - rbuf ) )
                    {
                        err = true;
                    }
                    delete[] rbuf;
            ...
            LeafNode *load_leaf_node ( int64_t id, bool prom )
            {
                ...
                if ( !db_.accept ( hbuf, hsiz, &visitor, false ) )
                {
                    return NULL;
                }
                ...
                if ( rit != ritend && !reccomp_ ( rec, *rit ) )
                {
                    ...
                    if ( vbuf == Visitor::REMOVE )
                    {
                        ...
                        if ( vbuf != Visitor::NOP )
                        {
                            ...
                            std::memcpy ( kbuf + rec->ksiz, vbuf, vsiz );
                            rec->vsiz = vsiz;
                            if ( node->size > psiz_ && recs.size() > 1 )
                            {
                                reorg = true;
                            }
                        }
            ...
            bool reorganize_tree ( LeafNode *node, int64_t *hist, int32_t hnum )
            {
                ...
                if ( node->size > psiz_ && node->recs.size() > 1 )
                {
                    ...
                    while ( true )
                    {
                        ...
                        if ( inode->size <= psiz_ || links.size() <= INLINKMIN )
                        {
                            break;
                        }
                    ...
                    if ( node->recs.empty() && hnum > 0 )
                    {
                        if ( !escape_cursors ( node->id, node->next ) )
                        {
                            return false;
                        }
            ...
            bool dump_meta()
            {
                ...
                wp += sizeof ( num );
                if ( !db_.set ( KCPDBMETAKEY, sizeof ( KCPDBMETAKEY ) - 1, head, sizeof ( head ) ) )
                {
                    return false;
                }
                trlcnt_ = lcnt_;
            ...
            bool begin_transaction_impl ( bool hard )
            {
                ...
                LeafSlot *lslot = lslots_ + idx;
                if ( lslot->warm->count() + lslot->hot->count() > 1 )
                {
                    flush_leaf_cache_part ( lslot );
                }
                InnerSlot *islot = islots_ + idx;
                if ( islot->warm->count() > 1 )
                {
                    flush_inner_cache_part ( islot );
                }
                if ( ( trlcnt_ != lcnt_ || count_ != trcount_ ) && !dump_meta() )
                {
                    return false;
                }
                if ( !db_.begin_transaction ( hard ) )
                {
                    return false;
                }
                return true;
            ...
            {
                _assert_ ( true );
                if ( !db_.begin_transaction ( autosync_ ) )
                {
                    return false;
                }
                bool err = false;
                ...
                int32_t idx = trclock_++ % SLOTNUM;
                LeafSlot *lslot = lslots_ + idx;
                if ( lslot->warm->count() + lslot->hot->count() > cnum )
                {
                    flush_leaf_cache_part ( lslot );
                }
                InnerSlot *islot = islots_ + idx;
                if ( islot->warm->count() > cnum )
                {
                    flush_inner_cache_part ( islot );
                }
                if ( !dump_meta() )
